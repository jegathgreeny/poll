...
class QuestionModelTests(TestCase):
    ...
    def test_old_question(self):
        """was_published_recently() returns False for questions whose published date is older than one day."""
        time = timezone.now() - datetime.timedelta(days=1, seconds=1)
        old_question = Question(published_date=time)
        self.assertIs(old_question.was_published_recently(), False)

    def test_recent_question(self):
        """was_published_recently() returns True for questions whose published date is within the last day."""
